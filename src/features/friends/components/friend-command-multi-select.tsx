...
import { getEligibleObserverMatchFriendsAction } from "../actions/actions";

type Friends = NonNullable<
  Awaited<ReturnType<typeof getEligibleObserverMatchFriendsAction>>
>["friends"];
...
export const FriendCommandMultiSelect = ({
  matchId,
  values,
  onValuesChange,
  onClick,
  ...
  matchId: string;
  values: PropValue[];
  onValuesChange: (values: PropValue[]) => void;
  ...
  const handleSearch = () => {
    startSearchTransition(async () => {
      const response = await getEligibleObserverMatchFriendsAction(
        matchId,
        search,
        DEFAULT_PAGE,
      );
  ...
  return (
    <div>
      <>
        ...
        >
          {values.length ? (
            <span className="text-left font-medium flex-1 min-w-0 truncate">
              {values.length} {values.length === 1 ? "friend" : "friends"}{" "}
              selected
            </span>
          ) : (
              ...
              {!isSearchPending && friends.length ? (
                <CommandGroup className="mt-2">
                  <div className="flex flex-col gap-2">
                    {friends.map((f) => {
                      const { user, ...friend } = f;
                      const isSelected = values.find((f) => f.id === friend.id);
